...
const AuthScreen = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // EXPLICIT UNLOCK: Ensure immediate state sync for UI transition
      const { unlockVault } = await import("@/store/vault.store").then((m) => ({
        unlockVault: m.useVaultStore.getState().unlockVault,
      }));
      unlockVault();

      // Auth state is handled by vault.store - no explicit close needed
  ...
  // This is now the app entry point when not authenticated.
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
          ...
          <div className="w-full min-h-[300px]">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="w-full"
              >
                <TabsContent value="login" className="mt-0 space-y-5 w-full">
                  <form
                ...
                <TabsContent value="register" className="mt-0 space-y-5 w-full">
                  <form
